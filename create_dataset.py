...
class DatasetFactory:
    ...
    def __init__(self):
        ...
        parser.add_argument('-t', '--use-tba-data', action='store_true', help='flag indicating whether to use TBA sort order data in the dataset')
        ...
        parser.add_argument('-c', '--classification', action='store_true', help='flag indicating whether the dataset is for winner classification instead of actual score prediction')
        list_group = parser.add_mutually_exclusive_group()
        list_group.add_argument('-b', '--event-blacklist', nargs='+', type=str, help='list of official names of events on TBA record to exclude')
        list_group.add_argument('-w', '--event-whitelist', nargs='+', type=str, help='list of official names of events on TBA record to use only')
        measures_group = parser.add_mutually_exclusive_group()
        measures_group.add_argument('-s', '--sum', action='store_true', help="take the sum of each team's statistics for the entire alliance to reduce training features")
        measures_group.add_argument('-a', '--average', action='store_true', help="take the average of each team's statistics for the entire alliance to reduce training features")
        measures_group.add_argument('-m', '--median', action='store_true', help="take the median of each team's statistics for the entire alliance to reduce training features")
    ...
    def create_dataset(self):
                ...
                try:
                    ...
                    for team_key in red_team_keys:
                        ...
                        if self.use_tba_data:
                            ...
                            red_tba_data.append(team_status['qual']['ranking']['sort_orders'][:relevant_sort_order_statistics])
                    ...
                    for team_key in blue_team_keys:
                        ...
                        if self.use_tba_data:
                            blue_tba_data.append(team_status['qual']['ranking']['sort_orders'][:relevant_sort_order_statistics])
        ...
        np.savez(self.out_filepath, header=np.array({'is_classification' : self.is_classification}), x=np.array(x), y=np.array(y))
